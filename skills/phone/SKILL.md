...
2. Select the General admin, user-managed, or S2S Marketplace app model and verify the exact Phone scopes and event subscriptions.
3. Confirm the actor, account settings, phone entitlements, and OAuth scopes before implementation.
4. Keep call control, call records, number management, and webhook processing as separate modules.
5. For Smart Embed, validate postMessage event contracts, embedding constraints, and CRM state mapping.
6. Debug by checking scopes, phone license, role permissions, number ownership, webhook subscriptions, and event payload shape.
